...
local HoshinoShield = {
    ...
    init = function (self)
        ...
        events.ITEM_RENDER:register(function (item, mode)
            if item.id == "minecraft:shield" and mode ~= "HEAD" and self.hasShield then
                ...
                ModelAlias.alias.avatar.body.Shield:setVisible(true)
    ...
    setShield = function (self, value, shouldPlayShieldSound)
        if value and not self.hasShield then
            ModelAlias.alias.avatar.body.Shield:setParentType("Item")
        ...
        elseif not value and self.hasShield then
            ModelAlias.alias.avatar.body.Shield:setVisible(true)
            ModelAlias.alias.avatar.body.Shield:setParentType("None")
            ...
            ModelAlias.alias.avatar.body.Shield:setSecondaryRenderType("NONE")
        end
