    ...
    setArmState = function (self, right, left)
        ...
        if self.armState.right ~= self.armStatePrev.right then
            ...
            if shouldPreventDefault then
                ...
                elseif self.armState.right == "GUN_OFF_HAND" then
                    events.TICK:register(function ()
                        self:processArmWingCount()
                    end, "right_arm_tick")
                ...
                elseif self.armState.right == "CROSSBOW" then
                    events.TICK:register(function ()
                        if player:getActiveItem().id ~= "minecraft:crossbow" and self.armState.right == 3 then
                            --if self.parent.gun.currentGunPosition == "RIGHT" then //TODO: 銃スクリプト再実装後
        ...
        if self.armState.left ~= self.armStatePrev.left then
            ...
            if shouldPreventDefault then
                ...
                elseif self.armState.left == "GUN_OFF_HAND" then
                ...
                elseif self.armState.left == "CROSSBOW" then
                    --クロスボウ装填中
                    events.TICK:register(function ()
