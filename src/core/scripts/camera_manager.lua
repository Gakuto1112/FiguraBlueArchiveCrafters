---@class (exact) CameraManager カメラ制御全般を管理するクラス
        ...
        if enabled and events.RENDER:getRegisteredCount("camera_manager_render") == 0 then
            events.RENDER:register(function ()
                if renderer:isFirstPerson() then
                    ...
                    local baseVector = vectors.rotateAroundAxis(math.deg(math.asin(cameraDir.y)), 0, 0.145, 0, vectors.rotateAroundAxis(math.deg(math.atan2(cameraDir.z, cameraDir.x)) * -1 - 90, 1, 0, 0, 0, 1, 0))
